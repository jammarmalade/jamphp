...
class Template {
    ...
    private function compile_tpl() {
        ...
        $tpl = preg_replace_callback("/[\n\r\t]*\{eval\}\s*(<\!\-\-)*(.+?)(\-\->)*\s*\{\/eval\}[\n\r\t]*/is", function($matches) {
            return self::evaltags($matches[2]);
        }, $tpl);
        $tpl = preg_replace_callback("/[\n\r\t]*\{eval\s+(.+?)\s*\}[\n\r\t]*/is", function($matches) {
            return self::evaltags($matches[1]);
        }, $tpl);

        $tpl = preg_replace_callback("/$replace_var/s", function($matches) {
            return self::addquote('<?='.$matches[1].'?>');
        }, $tpl); //所有php变量添加php短标记
        $tpl = preg_replace_callback("/<\?\=<\?\=$replace_var\?>\?>/s", function($matches) {
            return self::addquote('<?='.$matches[1].'?>');
        }, $tpl); //若有两个短标记改为一个
        ...
        //引入模版的函数
        $tpl = preg_replace_callback("/[\n\r\t]*\{display\s+([a-z0-9_\/]+)\}[\n\r\t]*/is", function($matches) {
            return self::stripvtags("<? include display('".$matches[1]."'); ?>");
        }, $tpl);
    ...
    public function display($tplpath, $tplcachepath, $layout = '') {
        ...
        $tplcachemtime = file_exists($this->tplcachepath) ? filemtime($this->tplcachepath) : '';
