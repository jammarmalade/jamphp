...
$(function () {
    ...
    //搜索标签
    $("#tags_ipt_add").autocomplete('?m=blog&c=tag&a=search', {
        matchContains: true,
        delay: 500,
        cacheLength: 100,
        matchSubset: true,
...
function showLogin() {
    $.post('?m=blog&c=user&a=login', {}, function (d) {
        var d = eval('(' + d + ')');
...
function loginDo(){
    ...
    var url = '?m=blog&c=user&a=loginDo';
    _ajax(url, data, function (res) {
        callback_login(res);
    });
    return false;
...
function addTag(id, tagname, type) {
    ...
    var success = true;
    if (id == 0) {
        _ajax('?m=blog&c=tag&a=add', {tagname: tagname}, function (res) {
            if (res['status']) {
                id = res['data'];
            } else {
                showMessage(res['data']);
...
function addRelation(id, dotype) {
    ...
    _ajax('?m=blog&c=tag&a=relation', {'tagid': id, 'aid': aid,'dotype':dotype}, function (res) {
        if (res['status']) {
            //添加/删除关系成功
        } else {
            showMessage(res['data']);
